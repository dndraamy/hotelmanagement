<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            ...
            {{-- Info approver --}}
            @if ($pengajuanCuti->status_approval !== 'Pending' && $pengajuanCuti->approver)
            <hr class="border-stone-100">
            <div>
                <p class="text-[10px] uppercase tracking-widest text-stone-400 font-semibold mb-3">
                    Diproses Oleh
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-semibold text-hotel-dark">
                            {{ $pengajuanCuti->approver->nama_lengkap }}
                        </p>
                    </div>
                    <div>
    ...
    <div id="modalSetuju" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <form method="POST" action="{{ route('hrd.cuti.approve', $pengajuanCuti->id_cuti) }}">
                @csrf @method('PATCH')
    ...
    <div id="modalTolak" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <form method="POST" action="{{ route('hrd.cuti.reject', $pengajuanCuti->id_cuti) }}">
                @csrf @method('PATCH')
                <div class="p-6 border-b border-stone-100">
