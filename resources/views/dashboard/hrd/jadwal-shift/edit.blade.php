...
@section('content')
    ...
    <div class="py-8 px-4 sm:px-8" style="background:#FAF9F6; min-height:100vh; font-family:'Montserrat',sans-serif;">
        <div class="bg-white rounded-2xl shadow-sm p-6 max-w-lg mx-auto">
            ...
            <h1 class="text-xl font-bold mb-1" style="color:#1A1A1A;">Edit Jadwal Shift</h1>
            <p class="text-sm mb-4" style="color:#888;">Perbarui jadwal shift pegawai di bawah ini</p>

            <div class="text-sm px-3 py-2 rounded-lg mb-6"
                 style="background:#fdf3d0; color:#8a6d00;">
                ✏️ Mengedit jadwal ID #{{ $jadwal->id_jadwal }}
            </div>

            <hr style="border-color:#f0ede6;" class="mb-6">

            <form method="POST" action="{{ route('hrd.dashboard.hrd.jadwal-shift.update', $jadwal->id_jadwal) }}" class="space-y-5">
                @csrf @method('PUT')
                    ...
                    <select name="id_pegawai"
                        ...
                        @foreach($pegawais as $p)
                            <option value="{{ $p->id_pegawai }}"
                                {{ old('id_pegawai', $jadwal->id_pegawai) == $p->id_pegawai ? 'selected' : '' }}>
                                {{ $p->nama_lengkap }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="id_shift"
                        ...
                        @foreach($shifts as $s)
                            <option value="{{ $s->id_shift }}"
                                {{ old('id_shift', $jadwal->id_shift) == $s->id_shift ? 'selected' : '' }}>
